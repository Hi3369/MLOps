# MCP Servers: MLOps専門機能（旧設計・非推奨）

> ⚠️ **注意**: このディレクトリは**旧設計（6個の独立MCPサーバー）**用です。
>
> **新しい設計では、[../mcp_server/](../mcp_server/)（単数形）の統合MLOps MCPサーバーを使用してください。**
>
> このディレクトリは、設計の変遷を記録するために残されています。

---

このディレクトリには、MLOps システムの専門機能を提供する Model Context Protocol (MCP) サーバーの**旧設計**が含まれています。
