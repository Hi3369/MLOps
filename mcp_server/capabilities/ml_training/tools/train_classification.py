...
def train_classification(
    ...
    if validation_data_s3_uri:
        try:
            ...
            logger.info(f"Validation accuracy: {validation_score:.4f}")

            # オーバーフィッティング検出
            score_diff = train_score - validation_score
            if score_diff > 0.1:
                overfitting_warning = (
                    f"Potential overfitting detected: train_accuracy={train_score:.4f}, "
                    f"validation_accuracy={validation_score:.4f}, diff={score_diff:.4f}"
                )
    ...
    if validation_score is not None:
        result["training_results"]["validation_accuracy"] = float(validation_score)
    if overfitting_warning:
