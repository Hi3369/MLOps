"""
Collect Model Metrics Tool

モデルメトリクス収集ツール（精度、予測分布等）
"""
...
def collect_model_metrics(
    ...
    Returns:
        モデルメトリクス辞書
    """
    logger.info(f"Collecting model metrics for endpoint: {endpoint_name}")
    ...
    try:
        ...
        errors_4xx = metrics_data.get("Invocation4XXErrors", {})
        errors_5xx = metrics_data.get("Invocation5XXErrors", {})
        ...
        if invocations.get("available") and invocations["latest"].get("sum", 0) > 0:
            ...
            metrics_data["ErrorRates"] = {
                "available": True,
                "4xx_error_rate": error_rate_4xx,
                "5xx_error_rate": error_rate_5xx,
                "total_error_rate": error_rate_total,
                "total_invocations": total_invocations,
            }
        else:
            metrics_data["ErrorRates"] = {
                "available": False,
                "message": "Insufficient data to calculate error rates",
            }

        logger.info(f"Model metrics collected for endpoint: {endpoint_name}")

        return {
            "status": "success",
            "message": f"Model metrics collected for endpoint: {endpoint_name}",
            "metrics_info": {
        ...
        raise ValueError(f"Failed to collect model metrics: {e}")
    ...
    try:
        ...
        return {
            ...
            "aggregated": {
                ...
                "total": sum([dp.get("Sum", 0) for dp in sorted_datapoints]),
            },
        }
