...
- **依存関係**: pyproject.tomlにboto3, botocore等のAWS SDK依存を追加 (ca498f2)
- **ドキュメント**: docs/designs/iam_permissions.mdにIAM権限一覧を追加 (6e3c88b)

## 総評

MLOps関連のCapabilityは高品質に実装されています。デプロイメント、再学習管理、モデルレジストリの各機能が設計通りに動作します。依存関係とIAM権限ドキュメントが追加されました。
